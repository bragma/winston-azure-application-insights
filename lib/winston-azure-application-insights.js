...
var DEFAULT_MAX_RECURSION_LEVEL = 20;
var DEFAULT_MAX_RECURSION_BEHAVIOR = 'ignore'

function noOp(x) {
	return x;
}

function flattenObject(source, target, prefix, recursionConfig, recursionLevel) {
	Object.keys(source).forEach(function(key) {
		var sourceVal = source[key];
		var fullKey = prefix ? prefix + '_' + key : key;
		if (sourceVal && typeof sourceVal === "object") {
			if (recursionLevel < recursionConfig.maxRecursionLevel) {
				flattenObject(sourceVal, target, fullKey, recursionConfig, recursionLevel+1);
			} else if (recursionConfig.maxRecursionBehavior === 'throw') {
				throw new Error('Maximum depth of recursion reached');
			}
		} else {
...
function flattenMeta(meta) {
	var flat = {}
	flattenObject(meta, flat, null, this, 1);
	return flat;
}
...
var AzureApplicationInsightsLogger = function (options) {
	...
	if (this.formatMeta === "flat") {
		this.formatMeta = flattenMeta.bind({
			maxRecursionLevel: options.maxFlatMetaDepth || DEFAULT_MAX_RECURSION_LEVEL,
			maxRecursionBehavior: options.maxFlatMetaDepthBehavior || DEFAULT_MAX_RECURSION_BEHAVIOR
		});
	}
};
